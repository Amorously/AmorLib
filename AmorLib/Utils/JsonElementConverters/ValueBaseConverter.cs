    ...
    public override ValueBase Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        switch (reader.TokenType)
        {
            ...
            case JsonTokenType.StartObject:
                ...
                while (reader.Read())
                {
                    if (reader.TokenType == JsonTokenType.EndObject) return valueBase;
                    else if (reader.TokenType != JsonTokenType.PropertyName) throw new JsonException("Expected PropertyName token");
            ...
            case JsonTokenType.String:
                ...
                if (strValue.EndsWith("%", StringComparison.InvariantCulture) 
                    && float.TryParse(strValue[0..^1].TrimEnd(), NumberStyles.Float | NumberStyles.AllowThousands, CultureInfo.InvariantCulture, out var parsedPercent))
                {
                    return new ValueBase(parsedPercent / 100.0f, ValueMode.Rel, fromDefaultFlag);
                }
                else if (strValue.EqualsAny(true, "Unchanged", "Ignore", "Keep", "Original", "KeepOriginal"))
                {
